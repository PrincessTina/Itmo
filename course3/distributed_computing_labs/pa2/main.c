...
void sendSynchronizeMessage(Process *currentProcess, FILE *event_log, MessageType messageType) {
    ...
    switch (messageType) {
        case STARTED:
            ...
            sprintf(text, log_started_fmt, get_physical_time(), currentProcess->id, currentProcess->pid,
        ...
        case DONE:
            // Запись в text строки сообщения
            sprintf(text, log_done_fmt, get_physical_time(), currentProcess->id, currentProcess->balanceState.s_balance);
    ...
    switch (messageType) {
        case STARTED:
            fprintf(event_log, log_received_all_started_fmt, get_physical_time(), currentProcess->id); // Запись события в event_log
            printf(log_received_all_started_fmt, get_physical_time(), currentProcess->id); // Запись события на stdout
            break;
        case DONE:
            fprintf(event_log, log_received_all_done_fmt, get_physical_time(), currentProcess->id); // Запись события в event_log
            printf(log_received_all_done_fmt, get_physical_time(), currentProcess->id); // Запись события на stdout
            break;
        default:
            break;
    }
...
void handleTransferMessage(Process *currentProcess, FILE *event_log, Message *message) {
    ...
    const int time = get_physical_time();
    ...
    if (currentProcess->id == src) {
        ...
        currentProcess->balanceHistory.s_history[currentProcess->balanceHistory.s_history_len - 1] = currentProcess->balanceState;
...
/**
 * Заполняет историю в местах, для которых неизвестно время изменения
 * @param currentProcess
 */
void spreadBalanceHistoryIfNeeded(Process *currentProcess) {
    int lastHistoryTime = currentProcess->balanceHistory.s_history[currentProcess->balanceHistory.s_history_len - 1].s_time;
    int currentTime = get_physical_time();

    if (currentTime > lastHistoryTime) {
        currentProcess->balanceHistory.s_history[currentProcess->balanceHistory.s_history_len] =
                currentProcess->balanceHistory.s_history[currentProcess->balanceHistory.s_history_len - 1];
        currentProcess->balanceHistory.s_history[currentProcess->balanceHistory.s_history_len].s_time = currentTime;
...
void work(Process *currentProcess, FILE *event_log) {
    ...
    while (!stop) {
        spreadBalanceHistoryIfNeeded(currentProcess);
 ...
 * @return
 */
AllHistory listeningHistory(Process *currentProcess) {
    AllHistory history;
...
void initProcesses(Process *currentProcess, const int *balance) {
    for (int i = 1; i < currentProcess->processes_amount; i++) {
        ...
        if (pid == 0) {
            ...
            currentProcess->balanceState.s_time = get_physical_time();
...
int main(int argc, char *argv[]) {
    ...
    if (currentProcess.id == PARENT_ID) {
        AllHistory history = listeningHistory(&currentProcess);
