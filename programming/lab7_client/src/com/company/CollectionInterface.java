...
class CollectionInterface {
  ...
  private static void modifyWindow(Display display, Tree tree) {
    ...
    modify.addListener(SWT.Selection, (Event e) -> {
      ...
      try {
        ...
        if (ServerConnection.sendCommand("modify", shorty.toString(),
            people.get(Integer.parseInt(index.getText())).toString()).compareTo("1") == 0) {
      ...
      if (spaceJson.getEnabled()) {
        ...
        if (matcher.matches()) {
          ...
          try {
            ...
            errorMessageWindow(mainShell, ex);
          }
        }
      } else {
        try {
  ...
  private static void errorMessageWindow(Shell shell, Exception ex) {
    int style = SWT.APPLICATION_MODAL | SWT.OK;
