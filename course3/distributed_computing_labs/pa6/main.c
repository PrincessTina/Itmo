...
void initProcesses(Process *currentProcess) {
    for (int i = 1; i < currentProcess->processes_amount; i++) {
        ...
        if (pid == 0) {
            ...
            for (int j = 1; j < currentProcess->processes_amount; j++) {
                if (j == i) {
                    continue;
                }

                if (j > i) {
                    currentProcess->forks[j].presence = 1;
                    currentProcess->forks[j].dirty = 1;
                } else {
                    currentProcess->forks[j].token = 1;
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    currentProcess.finishedProcesses = calloc(sizeof(int), currentProcess.processes_amount);
    currentProcess.forks = calloc(sizeof(Fork), currentProcess.processes_amount);
